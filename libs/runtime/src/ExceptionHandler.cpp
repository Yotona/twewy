...
inline void DestroyVarLenArray(int apply, ExceptionInfo* info, ThrowContext* context) {
    ...
    regptr2 = *(unsigned char*)ap & REGPTR;
    ap += sizeof(char);
...
static CatchInfo* FindMostRecentException(ThrowContext* context, ExceptionInfo* info) {
    ...
    context->throwtype = (unsigned char*)catchinfo->typeinfo;
...
static int IsInSpecification(const char* extype, ex_specification* spec) {
    ...
    for (i = 0; i < spec->nspecs; i++) {
        ...
        if (__throw_catch_compare(extype, spectype, &dummy) != 0) {
            return 1;
        }
        specptr += 4;
    }
...
#pragma exception_magic
    ...
    try {
        ...
        if (IsInSpecification((const char*)((CatchInfo*)&__exception_magic)->typeinfo, &spec)) {
            throw;
        }
    ...
    for (action = CurrentAction(&iter);; action = NextAction(&iter)) {
        switch (action) {
            case EXACTION_CATCHBLOCK:
                ...
                if (__throw_catch_compare((const char*)context->throwtype, handler.catch_type, result_offset) == 0) {
                    continue;
                }
                break;
            case EXACTION_SPECIFICATION:
                DecodeSpecification(iter.info.action_pointer, &spec);
                if (IsInSpecification((char*)context->throwtype, &spec) == 0) {
