...
static const char* parse_format(const char* format_string, va_list* arg, print_format* format) {
    ...
    switch (c) {
        ...
        case 't':
            ...
            break;
        case 'z':
            f.argument_options = u32_argument;
            break;
        default:
            flag_found = 0;
            break;
    ...
    switch (c) {
...

        {
            if (f.argument_options == short_argument || f.argument_options == intmax_argument ||
                f.argument_options == u32_argument || f.argument_options == ptrdiff_argument ||
                f.argument_options == long_long_argument)
            {
                f.conversion_char = bad_conversion;
                break;
...
// Nonmatching: Minor differences in switch statement order
static int __pformatter(void* (*WriteProc)(void*, const char*, u32), void* WriteProcArg, const char* format_str, va_list arg) {
        ...
        switch (format.conversion_char) {
            ...
            case 'i':
                if (format.argument_options == long_argument)
                    ...
                    long_long_num = va_arg(arg, u64);
                else if (format.argument_options == u32_argument)
                ...
                if ((format.argument_options == long_long_argument) || (format.argument_options == intmax_argument)) {
                    if ((buff_ptr = longlong2str(long_long_num, buff + conversion_buff_size, format)) == NULL) {
                        goto conversion_error;
                    }
                } else if ((buff_ptr = long2str(long_num, buff + conversion_buff_size, format)) == NULL) {
                    goto conversion_error;
                }
            ...
            case 'X':
                ...
                if ((format.argument_options == long_long_argument) || (format.argument_options == intmax_argument)) {
                    if (!(buff_ptr = longlong2str(long_long_num, buff + conversion_buff_size, format)))
                        goto conversion_error;
            ...
            case 'n':
                ...
                switch (format.argument_options) {
                    ...
                    case intmax_argument:
                        *(u64*)buff_ptr = chars_written;
                        break;
                    case u32_argument:
                        *(u32*)buff_ptr = chars_written;
                        break;
...
void* __StringWrite(void* osc, const char* Buffer, u32 NumChars) {
    s32           CharsToBeWritten;
    void*         MemCpyResult;
...
u32 vsnprintf(char* s, u32 n, const char* format, va_list arg) {
    u32          end;
    __OutStrCtrl osc;
