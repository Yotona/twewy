...
void func_02017040(ADXT* adxt) {
    ...
    ADXRNA_SetPlaySw(adxt->audioRenderer, 0);
...
void adxt_Pause(ADXT* adxt, s32 pauseState) {
    ...
    if (pauseState == adxt->pauseFlag) {
        return;
    }

    ADXCRS_Lock();
    adxt->pauseFlag = pauseState;

    if ((stat - 3) < 2) {
        if (pauseState == TRUE) {
            ADXRNA_SetPlaySw(adxt->audioRenderer, FALSE);
        } else {
            ADXRNA_SetPlaySw(adxt->audioRenderer, TRUE);
            adxt->vsyncCount = adxt_vsync_cnt;
        }
        ...
        adxt->startTimeOffset = (f32)count / frequency * adxt_time_unit;
    }
    ADXCRS_Unlock();
}
