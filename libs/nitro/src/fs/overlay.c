...
u32 FS_GetOverlayRamSize(FSOverlay* info) {
    if (FS_IsOverlayCompressed(info)) {
        ...
        return info->textSize;
    }
}

void FS_InvalidateOverlay(FSOverlay* info) {
    void* addr      = info->addr;
    u32   ramSize   = info->textSize;
...
FS_FileIdentifier FS_GetOverlayIdentifier(FSOverlay* info) {
    FS_FileIdentifier chunk;

    chunk.record = &Rom_DefaultRecord;
    chunk.fileID = info->fileId;
    return chunk;
}

BOOL FS_LoadVerifyOverlay(FSOverlay* info, u32 param_2, u32 id, void* param_4, u32 param_5, u32 param_6, u32 param_7,
                          u32 param_8) {
...
BOOL FS_LoadOverlayInfo(FSOverlay* info, FSOverlay* arg1, u32 id) {
    ...
    return FS_LoadVerifyOverlay(info, arg1, id, &Rom_DefaultRecord, unkDetail50->unk_00, unkDetail50->unk_04,
                                unkDetail58->unk_00, unkDetail58->unk_04);
}

BOOL FS_LoadOverlayFile(FSOverlay* info) {
    FS_File file[1];
...
void FS_StartOverlay(FSOverlay* info) {
    u32 size = FS_GetOverlayRamSize(info);

    if (REG_MULTIBOOT_MODE == 2) {
        BOOL valid = FALSE;

        if ((info->flags & 2) != 0) {
            if (info->id < ((ARM9_DATA_END - ARM9_BSS_START) / 20)) {

                u32 bssAddr = (ARM9_BSS_START + 20 * info->id);
    ...
    DC_PurgeRange(info->addr, info->textSize);

    OverlayFunc* ctorStart = info->ctorStart;
    OverlayFunc* ctorEnd   = info->ctorEnd;
    for (; ctorStart < ctorEnd; ++ctorStart) {
        if (*ctorStart != NULL) {
            (**ctorStart)();
        }
    }
...
void FS_DestroyOverlay(FSOverlay* info) {
    while (TRUE) {
        DestructorChain* head = NULL;
        DestructorChain* tail = NULL;

        const u32 start = info->addr;
        const u32 end   = start + (info->textSize + info->bssSize);
...
BOOL FS_StopOverlay(FSOverlay* info) {
...
BOOL FS_LoadOverlay(FSOverlay* overlay, u32 overlayID) {
    ...
    if (FS_LoadOverlayInfo(&info, overlay, overlayID) == FALSE || FS_LoadOverlayFile(&info) == FALSE) {
        return FALSE;
    }
...
BOOL FS_UnloadOverlay(FSOverlay* overlay, u32 overlayID) {
    ...
    if (FS_LoadOverlayInfo(&info, overlay, overlayID) == FALSE || FS_StopOverlay(&info) == FALSE) {
        return FALSE;
    }
    return TRUE;
}
