...
static OverlayDispatcher AuxOverlayDispatcher;
static OverlayDispatcher UnusedDispatcher2;
static OverlayDispatcher UnusedDispatcher1;
s32*                     data_0206af24;

static void OvlDisp_Push(OverlayDispatcher* dispatcher, s32 overlayId, OverlayCB callback, void* state, s32 repeatCount);
...
static void OvlDisp_Init(OverlayDispatcher* dispatcher) {
    ...
    OvlDisp_Push(dispatcher, -0x7fffffff, OvlDisp_DummyCb, NULL, 0);
}
...
static void OvlDisp_Push(OverlayDispatcher* dispatcher, s32 overlayId, OverlayCB callback, void* state, s32 repeatCount) {
    ...
    dispatcher->gState = currentData->gState;
...
static void OvlDisp_Pop(OverlayTag* tag, OverlayDispatcher* dispatcher) {
    ...
    dispatcher->gState = dispatcher->stack[currentIndex - 1].gState;

    if (dispatcher->stackDepth == 0) {
        OvlDisp_Push(dispatcher, 0x80000001, OvlDisp_DummyCb, NULL, 0);
    }
...
static void* OvlDisp_ReplaceState(OverlayDispatcher* dispatcher, void* state) {
    void* prevState          = dispatcher->gState;
    dispatcher->data->gState = state;
    dispatcher->gState       = state;
    return prevState;
}
...
static void OvlDisp_Run(OverlayDispatcher* dispatcher) {
    do {
        dispatcher->unk_0C = 0;
        dispatcher->cb(dispatcher->gState);
...
void MainOvlDisp_Push(s32 overlayId, OverlayCB callback, void* state, s32 repeatCount) {
    OvlDisp_Push(&MainOverlayDispatcher, overlayId, callback, state, repeatCount);
}
...
void MainOvlDisp_Pop(OverlayTag* tag) {
    ...
    dispatcher->gState     = previousData->gState;
...
void MainOvlDisp_ReplaceTop(OverlayTag* tag, s32 overlayId, void* callback, void* state, s32 param_5) {
    ...
    MainOvlDisp_Push(overlayId, callback, state, param_5);
    *tag = local;
}
...
void* MainOvlDisp_SetState(void* state) {
    OvlDisp_ReplaceState(&MainOverlayDispatcher, state);
}
...
void DebugOvlDisp_Init(void) {
    OvlDisp_Init(&AuxOverlayDispatcher);
}

void DebugOvlDisp_Push(OverlayCB callback, void* state, s32 param_3) {
    OvlDisp_Push(&AuxOverlayDispatcher, 0, callback, state, param_3);
}

s32 DebugOvlDisp_Pop(void) {
    OverlayTag tag;

    OvlDisp_Pop(&tag, &AuxOverlayDispatcher);
    return tag.cb;
}
...
s32 DebugOvlDisp_ReplaceTop(OverlayCB callback, void* state, s32 param_3) {
    ...
    OvlDisp_ReplaceTop(&tag, &AuxOverlayDispatcher, 0, callback, state, param_3);
    return tag.cb;
}

BOOL DebugOvlDisp_IsStackAtBase(void) {
    return OvlDisp_IsStackAtBase(&AuxOverlayDispatcher);
}

void DebugOvlDisp_Run(void) {
    OvlDisp_Run(&AuxOverlayDispatcher);
}
